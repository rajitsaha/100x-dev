...
>
> **This deletes files in your projects.** `100xprism update` removes the `.windsurfrules`, `GEMINI.md`, `ANTIGRAVITY.md`, and `.github/copilot-instructions.md` it previously generated from every project in `~/.100xprism/tracked-projects`; `100xprism init` does the same for the project it runs in. Only files still carrying the unmodified `Generated by 100xprism` header are removed — if you edited one, it is kept. These files are normally committed, so removals appear as unstaged deletions for you to review.
