...
- **`100x-tokens`** — one offline, machine-wide dashboard at a single URL: the input/output/cache split, a startup-bloat meter, an *estimated* code-vs-logs-vs-output composition, and $ cost — by project, model, session, and skill. Claude Code and Codex provide exact local token counters; Cursor and Antigravity contribute project/session/activity coverage but are never assigned invented token cost because their local formats expose no counters. It auto-refreshes every 30 seconds and auto-starts on shell startup and `install` / `init` / `update` (opt out with `export PRISM_NO_DASHBOARD=1`).
