...
def emit_index(modules: list[dict], indent: str = "") -> str:
    ...
    for cat in sorted(by_cat.keys(), key=category_sort_key):
        ...
        for m in by_cat[cat]:
            ...
            # Truncate description to first sentence (or 140 chars)
            d = m["description"]
            d = d.split(". ", 1)[0]
            if len(d) > 140:
                d = d[:137] + "..."
            lines.append(f"{indent}- `{m['slug']}`{slash} — {d}")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def cmd_emit_claude_code():
    ...
    for module_dir in sorted(MODULES_DIR.glob("*")):
        ...
        fm, _body = split_frontmatter(skill_md.read_text())
        slash = fm.get("slash_command", "").lstrip("/")
        if slash:
            (commands_dir / f"{slash}.md").write_text(
                f"Use the `{fm.get('name', slug)}` skill.\n\n$ARGUMENTS\n"
            )
            cmd_count += 1
