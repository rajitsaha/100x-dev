...
| `add` writes `platforms` wholesale | `merge_platforms()` unions obligations per platform (see the obligation-set row below) | A failed retry could otherwise downgrade a platform and drop the removal transition for a mutation that really happened. An intermediate fix used a ranked maximum; that was itself wrong, because the obligations are independent rather than ordered |
