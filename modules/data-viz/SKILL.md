...
**Always pass color-blind simulation** for protanopia + deuteranopia. Never red/green as the only signal — pair with shape, position, or label.

### Semantic colors

**Up / good / increase** — green ONLY if culturally appropriate (in financial contexts in CN/JP, red = up); default to blue for "up" in mixed audiences. **Down / bad / decrease** — red, with pattern + label backup. **Neutral / no change** — gray, not yellow. **Forecast / projected** — same hue as actuals, dashed line or hatch fill.

### Dark mode

Don't invert sequential ramps — use a ramp designed for dark backgrounds (viridis works on both). Lift saturation 5–10% to compensate for reduced contrast.

## Dashboard Layout Heuristics

### F-pattern or Z-pattern?
**F** — analyst dashboards (lots of detail, scanned top-left to bottom). **Z** — executive dashboards (3–7 KPIs, glanceable).

### The 5-second rule
The user must extract the headline in 5 seconds; otherwise the lede is buried — re-rank.
...
### Filters
**Date range** — top-right, always; default to last 30 days unless data has a clearer natural window. **Other filters** — left rail if persistent, top strip if transient. **Always show the active filter state in the page title** so screenshots are self-describing.
...
A chart is not done until all four exist: 1. **Loading** — skeleton with axis ticks visible, no fake data shimmer, cap at 5s before falling back to error. 2. **Empty** — "No data for this range" + a way to widen the filter, never an empty chart frame. 3. **Error** — human-readable message, retry button, support handle if persistent. 4. **Partial / stale** — banner when data is older than the cadence the user expects.

## Interactivity Defaults

**Tooltip** — on hover/focus, all encoded values, keyboard-accessible via Tab. **Crosshair** — time series, vertical line on hover with snapped points. **Legend** — clickable to toggle series, state persists per session. **Zoom** — only for time series > 1 year or scatter > 5k points, always with a reset. **Brush** — linked dashboards, broadcast selection to siblings.

## Accessibility for Charts

Every chart needs a `<figure>` with `<figcaption>` summarizing the takeaway in one sentence, plus a **data table fallback** behind a "View as table" disclosure. Color is never the only encoding — pair with shape, label, or position. Tooltips reachable via keyboard (Tab → arrow keys to traverse points). Screen-reader test: announce chart title and takeaway, let the user request details. Pass **a11y-auditor** criteria for contrast on every line, label, and grid.

## Performance Rules

SVG up to ~5k DOM nodes, Canvas above; throttle hover handlers with `requestAnimationFrame`; memoize scales, recompute only when data or dimensions change; streaming charts use a ring buffer capped to N visible points; disable animation on first paint when n > 200 points.
...
Per dashboard, also deliver a layout sketch (ASCII or Figma frame) showing chart placement, headline metric strip, filter positions, and density budget per breakpoint (≥ 1280, 768–1279, < 768).

## Anti-Patterns to Cut Immediately

Pie chart with more than 3 slices; truncated y-axis on a bar chart making small differences look huge; dual y-axes implying correlation by visual coincidence; rainbow palette for ordinal data; 3D charts; "number go up" tile with no comparison value (vs prior period, vs target); tile titles like "Stats" or "Overview" that don't name the metric; tooltip as the only place units are shown; charts with no figcaption; re-rendering 50k points on every hover; a dashboard that asks "what would you like to see?" instead of answering.

## Output Goal

A frontend engineer can implement every chart from the spec with library + props chosen; a product manager can read each chart's `question:` line and agree it answers the decision they care about; an accessibility reviewer finds a fallback table and figcaption on every chart.
