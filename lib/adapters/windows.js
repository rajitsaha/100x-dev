...
function emitCursorRules(modulesDir, projectPath) {
  ...
  const modules = listModules(modulesDir)
  for (const m of modules) {
    const alwaysApply = m.tier === 'core' ? 'true' : 'false'
    const fm = [
      '---',
      ...
      '---',
      '',
      `<!-- generated by 100xprism from modules/${m.slug}/SKILL.md -->`,
      '',
    ]
    const tier = tierAnnotation(m.model)
    if (tier) fm.push(tier, '')
    fs.writeFileSync(path.join(rulesDir, `${m.slug}.mdc`), fm.join('\n') + m.body.replace(/^\n+/, ''))
  }
...
function pruneDeprecatedArtifacts(projectPath, backupRoot) {
  ...
  // itself lossy (C:\a_b\c and C:\a\b_c both collapse), so a hash of the exact
  // path is appended — the readable part is for humans, the hash makes it injective.
  const slot = realProject.replace(/^[/\\]/, '').replace(/[/\\:]/g, '_') + '-' +
    require('crypto').createHash('sha1').update(realProject).digest('hex').slice(0, 8)
