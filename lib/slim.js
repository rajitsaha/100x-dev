...
function slimProject(projectPath, { dryRun }) {
  ...
  if (existing) {
    ...
    console.log(`  ${label}: would set profiles → ${profiles.join(', ')}`)
  } else {
    writeProjectProfiles(projectPath, profiles)
    const emitted = reemitProject(projectPath)
    console.log(`  ${label}: profiles → ${profiles.join(', ')}` +
...
function run(args) {
  ...
  if (targets.length) {
    ...
    for (const t of targets) slimProject(t, { dryRun })
  }

  console.log('\n  Undo: `100xprism slim --skills=all`, or set "profiles": ["all"] in a ' +
