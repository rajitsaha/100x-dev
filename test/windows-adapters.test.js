...
test('emitCursorRules prunes its own stale rules but keeps user-authored ones', () => {
  const modulesDir = makeTmpDir()
  const projectDir = makeTmpDir()
  const rulesDir = path.join(projectDir, '.cursor', 'rules')
  fs.mkdirSync(rulesDir, { recursive: true })
  fs.writeFileSync(path.join(rulesDir, 'dropped.mdc'), '---\ndescription: old\n---\n\n<!-- generated by 100xprism from modules/dropped/SKILL.md -->\n')
  ...
  assert.ok(!fs.existsSync(path.join(rulesDir, 'dropped.mdc')), 'removed a rule we previously generated')
  assert.ok(fs.existsSync(path.join(rulesDir, 'mine.mdc')), 'preserved a user-authored rule')
  assert.ok(fs.existsSync(path.join(rulesDir, 'gate.mdc')), 'wrote the current module')
})
